...
background-repeat: no-repeat;">
  ...
  `,"");d.innerHTML=t}function Q(e){h.innerHTML="";for(let t=1;t<=e&&!(t>3);t++){const o=document.createElement("button");o.textContent=t,t===1&&o.classList.add("active"),o.addEventListener("click",n=>{P(h,n.target),he(t)}),h.appendChild(o)}}async function he(e){a=e;try{w.scrollIntoView({behavior:"smooth"});const t=await l(`filters?filter=${i}`,{limit:k,page:a});T(t.results)}catch(t){console.error(t)}}async function ge(e){const t=e.target.dataset.filter,o=e.target.dataset.name;if(t){i=t,y=o,a=1,w.scrollIntoView({behavior:"smooth"}),G([d]),K(C);try{const n=await l(`exercises?${i}=${y}`,{limit:x,page:a});d.innerHTML="",h.innerHTML="",O(n.results),pe(n.totalPages)}catch(n){console.log(n)}}}function O(e){const t=e.reduce((o,{rating:n,target:r,bodyPart:s,burnedCalories:u,name:ne,_id:se})=>(o+=`<li class="card-workout-item" id=${se}>
                <div class="card-menu">
