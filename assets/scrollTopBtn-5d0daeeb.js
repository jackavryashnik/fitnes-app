...
background-repeat: no-repeat;">
  ...
  `,"");m.innerHTML=t}function oe(e){y.innerHTML="";for(let t=1;t<=e&&!(t>3);t++){const o=document.createElement("button");o.textContent=t,t===1&&o.classList.add("active"),o.addEventListener("click",r=>{H(y,r.target),ye(t)}),y.appendChild(o)}}async function ye(e){i=e;try{L.scrollIntoView({behavior:"smooth"});const t=await f(`filters?filter=${c}`,{limit:S,page:i});A(t.results)}catch(t){console.error(t)}}async function we(e){const t=e.target.dataset.filter,o=e.target.dataset.name;if(t){c=t,x=o,i=1,L.scrollIntoView({behavior:"smooth"}),_([m]),ee(W);try{const r=await f(`exercises?${c}=${x}`,{limit:q,page:i});m.innerHTML="",y.innerHTML="",N(r.results),ve(r.totalPages)}catch(r){console.log(r)}}}function N(e){const t=e.reduce((o,{rating:r,target:n,bodyPart:s,burnedCalories:l,name:w,_id:h})=>(o+=`<li class="card-workout-item" id=${h}>
                <div class="card-menu">
            ...
            </li>`,o),"");b.innerHTML=t}function ve(e){p.innerHTML="";for(let t=1;t<=e&&!(t>3);t++){const o=document.createElement("button");o.textContent=t,t===1&&o.classList.add("active"),o.addEventListener("click",r=>{H(p,r.target),ke(t)}),p.appendChild(o)}}async function ke(e){i=e;try{L.scrollIntoView({behavior:"smooth"});const t=await f(`exercises?${c}=${x}`,{limit:q,page:i});N(t.results)}catch(t){console.error(t)}}const ne=window.location.pathname,be=ne.lastIndexOf("/"),G=ne.substring(be);if(G==="/index.html"||G==="/"){const e=document.querySelector(".footer-form");e.addEventListener("submit",function(t){t.preventDefault(),xe(),e.reset()})}async function xe(){const e=document.querySelector(".footer-form"),r={email:new FormData(e).get("email")};try{const s=(await B.post(e.action,r,{headers:{"Content-Type":"application/json"}})).data;s&&s.message&&qe()}catch(n){n.response&&n.response.status!==409&&Le(),n.response&&n.response.status===409&&Se()}}function Le(){F.fire({title:"Bad request",text:"Something went wrong.",icon:"error"})}function Se(){F.fire({title:"Warning!",text:"Subscription already exists",icon:"warning"})}function qe(){F.fire({title:"Good job!",text:"We're excited to have you on board! 🎉 Thank you for subscribing to new exercises on Energy Flow. You've just taken a significant step towards improving your fitness and well-being.",icon:"success"})}function Ee(){const e=document.querySelector(".foter-privacy-policy"),t=document.querySelector(".footer-terms-of-service"),o=document.querySelector(".modal-overlay"),r=document.querySelector(".privacy-modal-overlay");e.addEventListener("click",()=>r.classList.add("is-open")),t.addEventListener("click",()=>o.classList.add("is-open")),r.addEventListener("click",()=>r.classList.remove("is-open")),o.addEventListener("click",()=>o.classList.remove("is-open"))}const re=window.location.pathname,$e=re.lastIndexOf("/"),K=re.substring($e);(K==="/index.html"||K==="/")&&Ee();document.addEventListener("DOMContentLoaded",function(){const e=document.querySelector(".favorites-list"),t=document.querySelector(".favorites-pagination-block"),o=document.querySelector(".message-info"),r=document.querySelector(".favorites-contanier-block");function n(){window.innerWidth<=767&&s()?t.style.display="flex":t.style.display="none"}function s(){return e&&e.offsetParent!==null}function l(){const h=window.innerWidth<=767&&s()?6:e.children.length;if(window.innerWidth>767&&s()){for(let a=0;a<e.children.length;a++)e.children[a].style.display="block";return}Math.ceil(e.children.length/h);let E=1;function U(a){const $=(a-1)*h,I=$+h;for(let d=0;d<e.children.length;d++)e.children[d].style.display="none";for(let d=$;d<I&&d<e.children.length;d++)e.children[d].style.display="block"}U(E),t.addEventListener("click",function(a){a.target.tagName==="BUTTON"&&(t.querySelectorAll("button").forEach(I=>I.classList.remove("active-btn")),a.target.classList.add("active-btn"),E=parseInt(a.target.textContent),U(E))})}s()?o.style.display="none":o.style.display="block";function w(){window.matchMedia("(min-width: 768px)").matches?r.style.overflowY="scroll":r.style.overflowY="visible",window.matchMedia("(min-width: 1440px)").matches?r.style.maxHeight="480px":r.style.maxHeight="none"}w(),n(),window.addEventListener("resize",function(){n(),l(),w()}),s()&&e.children.length>=6&&l()});const k="quoteDay",g=JSON.parse(localStorage.getItem(k)),P=document.querySelector(".quote-container-text"),R=document.querySelector(".quote-container-author"),T=new Date,v={date:T.getDate(),month:T.getMonth(),year:T.getFullYear()};!localStorage.getItem(k)||v.date!=g.date.date&&v.month!=g.date.month&&v.year!=g.date.year?(localStorage.removeItem(k),f("quote").then(e=>{P.textContent=e.quote,R.textContent=e.author,localStorage.setItem(k,JSON.stringify({author:e.author,quote:e.quote,date:v}))}).catch(e=>{console.log(e),P.textContent="Sorry, there was an error on the server"})):(P.textContent=g.quote,R.textContent=g.author);const se=window.location.pathname,Ie=se.lastIndexOf("/"),Q=se.substring(Ie);let C,ie,ae,ce;(Q==="/index.html"||Q==="/")&&(C=document.querySelector(".form-search-exercises"),ie=document.querySelector(".cards-workout-list"),ae=document.querySelector(".pagination-cards-workout-box"),ce=C.elements.search,C.addEventListener("submit",Me));async function Me(e){ie.innerHTML="",ae.innerHTML="",e.preventDefault();try{const t=await f(`exercises?${c}=${x}`,{page:i,limit:q,keyword:ce.value});N(t.results)}catch(t){console.log(t)}}const D=document.querySelector(".scroll-top-btn");function Pe(){window.scrollY>700?D.classList.remove("is-hidden"):window.scrollY<700&&D.classList.add("is-hidden")}window.onscroll=Pe;D.onclick=()=>window.scrollTo(0,0);
//# sourceMappingURL=scrollTopBtn-5d0daeeb.js.map
